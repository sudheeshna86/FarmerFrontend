...
export default function Offers() {
  ...
  const fetchOffers = async (requestedPage = page, status = statusFilter) => {
    try {
      setLoading(true);
      const params = { page: requestedPage, limit: pageSize, status };
      if (searchTerm) params.search = searchTerm;
      const data = await getFarmerOffers(params);
      ...
      <div className="card p-3 mb-4 shadow-sm">
        <div className="row g-3 align-items-end">
          <div className="col-md-5">
            <label className="form-label small mb-1">Search offers</label>
            <input
              className="form-control"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              placeholder="Search by buyer, crop, or location"
            />
          </div>
